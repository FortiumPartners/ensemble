...
**2. Problem Space Interview**
   Gather core product context through targeted questions

   - Ask these questions (skip any already answered in the product description):
   - 1. What problem does this solve? Who feels the pain today?
   - 2. Who are the primary users? (roles, not names)
   - 3. What does success look like? (specific metrics if possible)
   - 4. What constraints exist? (budget, timeline, tech stack, compliance)
   - 5. What existing solutions have been tried or considered?
   - Wait for answers before proceeding -- do not generate requirements from assumptions
   - Summarize what you heard back to the user in 3-4 sentences for confirmation

**3. Creative Elicitation**
   Use structured techniques to surface hidden and edge-case requirements

   - SCAMPER the core idea: ask the user 2-3 targeted questions from these angles:
   -   - Substitute: What if a key component were replaced? (e.g., 'What if users couldn't use a browser?')
   -   - Combine: What adjacent problems could this solve simultaneously?
   -   - Adapt: What existing product does this most resemble, and where must it differ?
   -   - Eliminate: What's the simplest version that still solves the core problem?
   - 'What could go wrong?' exercise: brainstorm 3-5 failure scenarios with the user (data loss, abuse, scale issues, accessibility gaps)
   - Record insights from both techniques -- these become requirements or risk flags later
   - For LIGHT depth: ask 2 SCAMPER questions and 2 failure scenarios
   - For DEEP depth: run full SCAMPER sweep and 5+ failure scenarios
...
**2. Issue Resolution**
   Present issues to the user and incorporate their decisions

   - Present the issues as a numbered list with recommended resolutions
   - Ask the user to approve, modify, or reject each recommendation
   - Update the PRD based on user decisions
