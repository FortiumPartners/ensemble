...
description: >-
  Create a Technical Requirements Document (TRD) from a Product Requirements
  Document (PRD). Performs PRD validation, architecture design with
...
---
<!-- Command: ensemble-create-trd | Version: 3.4.0 -->
...
> - Every REQ-NNN acceptance-criteria set MUST decompose into multiple Given/When/Then AC-NNN-M scenarios (happy path, edge case, error/negative case at minimum) -- each AC-NNN-M becomes exactly one BDD/Gherkin scenario consumed by reqnroll-cli during implementation. This command SPECIFIES those scenarios; it does NOT generate .feature files or run codegen (see DO NOT implement/build/execute constraint above).
...
4. Otherwise check that every functional REQ-NNN has at least 3 AC-NNN-M sub-items spanning happy path, edge case, and error/negative case -- these are the minimum BDD scenario set reqnroll-cli will scaffold per requirement. Flag any REQ-NNN with fewer than 3 as insufficient BDD coverage and note it in the readiness scorecard.
...
4. Classify companion artifact domains and record the result once as COMPANION_DOMAINS for later output steps: add `data-model` when requirements mention persistence changes such as entities, schemas, database tables, migrations, backfills, relationships, records, validation rules, ownership, privacy/security for stored data, or durable state changes.
5. Do NOT add `data-model` for incidental data mentions, read-only display data, analytics labels, report fields, request/response examples, or documentation-only references unless another persistence/schema signal is present.
6. Add `research` when requirements require comparative technology decisions, vendor/tool selection, integration options, architecture alternatives, dependency tradeoffs, or external service evaluation that should be reviewable independently from the TRD.
7. Do NOT add `research` for routine brownfield architecture description, straightforward use of an already-selected stack, or implementation notes that do not compare options or justify a technology/integration decision.
8. If no companion domains are detected, record COMPANION_DOMAINS as empty; later steps must not create empty companion placeholder files.
9. Determine if project is greenfield or brownfield (check for existing codebase)
10. Summarize domain coverage, COMPANION_DOMAINS, and gaps
...
**Actions:**
1. Resolve TRD_GRAPH_CLI per the tool-path-resolution skill (packages/development/skills/tool-path-resolution/SKILL.md) for packages/development/lib/trd-graph-cli.js. If none of the 4 tiers resolve, print error and HALT.
...
5. Ask user to choose one option or combine elements before proceeding. If --foreman is set, skip the ask -- automatically select Option C (best fit for existing codebase, or balanced approach for greenfield) as the recommended default, and print 'Foreman mode: auto-selected Option C (<summary>)' before continuing.
...
### Step 2: Test Task Generation (BDD)

Generate a paired BDD/Reqnroll test task, carrying multiple Gherkin scenarios, for every user-facing implementation task

**Actions:**
1. For every user-facing TRD-NNN implementation task, generate a TRD-NNN-TEST task
2. Every TRD-NNN-TEST line MUST begin with the same checkbox-prefix requirement as implementation tasks -- `- [ ] ` (or `- [x] ` if already complete) -- immediately before **TRD-NNN-TEST**. Every TRD-NNN-TEST task generated by this phase is a BDD/Reqnroll acceptance test by construction -- implement-trd/implement-trd-beads route any task whose id matches `-TEST` through the reqnroll BDD path (no separate tag needed; the existing TRD-CLI isTest field is the routing signal).
3. Test tasks include: [verifies TRD-NNN] [satisfies REQ-NNN] [depends: TRD-NNN] annotations, plus a `Validates PRD ACs: AC-NNN-M, AC-NNN-M, ...` field listing every AC-NNN-M the task covers
4. Under each TRD-NNN-TEST task body, add a `Test AC:` checklist section (the same labeled-checklist format implementation tasks already use for `Implementation AC:`) with one `- [ ] Scenario: <short title> -- Given <...>, When <...>, Then <...>` line per AC-NNN-M in its Validates PRD ACs field -- one Gherkin scenario per AC, not one scenario per task, and each item MUST stay on a single line (the TRD/beads parsers read Test AC as a flat checklist, one item per line). Indent each `- [ ] Scenario: ...` line strictly deeper than the `Test AC:` label line (matching Implementation AC's existing nesting) -- items at the same or shallower indent as the label are silently dropped by the parser. This is the ONLY place scenario text goes -- do not also add a separate '## Scenarios' heading; the parser does not read one and it will be silently dropped by TRD-CLI/scaffold-plan on the beads path.
5. Because the Acceptance Criteria Review phase requires >=3 AC-NNN-M per functional REQ-NNN (happy path, edge case, error/negative case), every TRD-NNN-TEST task therefore carries >=3 Test AC Scenario checklist items -- multiple BDD tests per acceptance criterion, never a single generic 'covers happy path and edge case' placeholder.
6. Note in the task body that implement-trd/implement-trd-beads will run reqnroll-cli generate-bindings against the PRD to scaffold these scenarios as .feature files + Pending() step stubs, then delegate reqnroll-binding-specialist to fill bodies test-first (see packages/reqnroll/skills/reqnroll/SKILL.md) -- this command only specifies the scenario titles and their Given/When/Then text, it does not scaffold or run codegen
7. Link test tasks to the PRD acceptance criteria they validate
...
4. Verify every TRD-NNN-TEST task's `Test AC:` checklist has one `- [ ] Scenario: ...` item per AC-NNN-M in its Validates PRD ACs field, with no AC left unscenario'd and no checklist item unlinked to an AC
5. Document any testability issues with recommended resolutions
...
10. Stable rerun rule: recomputing from the same PRD correlation id and slug must target the same companion paths; overwrite/update only the detected companion files and do not create stale placeholder artifacts for domains that are not detected.
11. When companion artifacts exist, include a `## Companion Artifacts` section in the TRD with relative links only to files actually generated; when COMPANION_DOMAINS is empty, either omit the section or state `No companion artifacts generated.` with no broken links.
12. When a task depends on a research or data-model decision, reference the companion artifact path and relevant REQ/AC ids instead of copying the full companion artifact body into the TRD.
