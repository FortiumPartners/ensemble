...
**1. Handle Special Arguments**
   ...
   - Parse $ARGUMENTS for .md paths. If two or more TRD paths are present: set COMBINED_WORKSTREAM_MODE=true, set SOURCE_TRD_PATHS to the ordered path list, print 'Combined workstream mode: preserving PRD/TRD traceability with separate TRD epics under one release train', and list every source TRD. If exactly one TRD path is present: set COMBINED_WORKSTREAM_MODE=false and preserve existing single-TRD behavior exactly.
   ...
   -        '================================='
   -   5. EXIT
   - If $ARGUMENTS contains '--status' AND COMBINED_WORKSTREAM_MODE=true: run br list --status=open --json and node "$TRD_CLI" workstream-status --issues-json <captured-json-file> (include --workstream <slug> if known); print release train progress, each TRD epic progress, blocked items, ready items, and parallel-safe streams; EXIT
...
**3. Root Epic Creation**
   ...
   - If COMBINED_WORKSTREAM_MODE=true: run node "$TRD_CLI" workstream-plan <SOURCE_TRD_PATHS...>$( [ "$ENSEMBLE_USE_STACKED_PRS" = true ] && echo ' --stacked' ) and parse {ok,workstreamSlug,releaseTrain,trdEpics,scaffoldPlans,crossTrd}. Create or reuse releaseTrain first, then one TRD root epic per trdEpics entry, then execute each scaffoldPlans[n].plan using the existing single-TRD PLAN executor. Preserve each TRD-local PR/story/task hierarchy under its TRD epic. If crossTrd.errors is non-empty or ok=false: print each ambiguity/conflict and ask the user how to resolve before creating affected dependency edges; do not guess.
...
**7. Dependency Encoding**
   ...
   - If COMBINED_WORKSTREAM_MODE=true: also execute WORKSTREAM_PLAN.deps and crossTrd.edges after local PLAN.deps. For each crossTrd edge, resolve blockerId and blockedId across the global title-prefix map, run `br dep add <blocked_bead_id> <blocker_bead_id>`, and add metadata comment '<edge.metadata>' to the blocked bead. If br reports a cycle or contradictory direction, print the exact edge and ask user to retry, skip, reverse, or abort; never auto-resolve.
...
**8. BV Execution Planning**
   ...
   -   Run: INSIGHTS_OUTPUT=$(bv --robot-insights --format toon) — capture graph health; if cycles, unexpected blockers, or priority/order mismatches are reported, print them and ask user how to proceed before execution. Never invoke bare bv; only --robot-* flags.
...
**9. Scaffold Summary and BV Analysis**
   ...
   - If COMBINED_WORKSTREAM_MODE=true: print 'Combined workstream mode: stacked PRs enabled' when STACKED_PRS=true; always print release train ID/name, all source TRD epics, per-TRD task counts, and cross-TRD dependency count.
