...
**1. Execution Loop**
   ...
   - DELEGATE TO TRD-EXECUTE ENGINE (TEAM_MODE=false):
   -   Delegate to: /ensemble:trd-execute "implement TRD <TRD_SLUG>" --trd "<TRD_FILE_PATH>" --strategy <STRATEGY> --max-parallel <MAX_PARALLEL>
   -   The trd-execute engine runs the full single-agent DRAIN LOOP: trd-cli phase ordering, bv primary dispatch with br ready fallback, implement-bead-worker execution, TRD checkbox sync, Quality Gate, and Completion.
   -   After delegation: RETURN from Execute phase — implement-trd-beads is done.
   - 
   - EXIT: Execute phase delegated to trd-execute for TEAM_MODE=false. Steps 2–10 are TEAM_MODE=true helpers only.
