...
**1. TRD Document Generation**
   ...
   - Generate a collision-resistant micro UUID for the document id: 8 lowercase hex characters from a UUID/random source. Do NOT scan for highest TRD sequence number or increment NNN; teams create TRDs concurrently.
   - File naming must be docs/TRD/TRD-YYYY-<micro_uuid>-<slug>.md — same slug as beads path, NO `-foreman` suffix, micro_uuid is 8 lowercase hex chars
