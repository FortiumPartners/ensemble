...
- Return: files changed, refactoring summary

**app-debugger** (Debugging Specialist):
- Analyze test failures using systematic methodology (5 Whys)
- Check if problem matches documented risks in TRD
- Implement targeted fixes for root cause
- Do NOT execute tests - return to VERIFY stage for that
- Return: files fixed, root cause analysis, risk match (if any)

**code-reviewer** (Quality & Security Specialist):
- Perform OWASP Top 10 security review
- Verify code quality and complexity
- Check Definition of Done (DoD) criteria
- Return: APPROVED, APPROVED_WITH_RECOMMENDATIONS, or REJECTED with specific issues

This separation ensures proper RED-GREEN-REFACTOR discipline for TDD.

### Stage Outputs (Handoff Contract)

Each stage returns specific outputs that the next stage requires:

| Stage | Agent | Returns | Used By |
|-------|-------|---------|---------|
| VERIFY-RED | verify-app | test_files[], failure_messages | IMPLEMENT (TDD only) |
...
| REVIEW | code-reviewer | decision (APPROVED/REJECTED), issues[], recommendations[] | UPDATE or IMPLEMENT |

**Handoff Rules:**
- Each stage MUST wait for the previous stage to complete before starting
- Failed VERIFY returns to DEBUG (not IMPLEMENT) for analysis
- REJECTED review returns to the SAME implementer that did the original work
- Visual verification failures return to frontend-implementer with specific feedback
...
When the task is delegated to `frontend-implementer`, include additional V0 and visual context:

```xml
<ui_context>
  <v0_generation>
    <catalog_reference>design/11-ui-component-catalog.md</catalog_reference>
    <design_spec>design/16-ui-design-specification.md</design_spec>
    <wireframes>design/02-views-and-wireframes.md</wireframes>
    <tokens>design/17-shared-design-tokens.md</tokens>
  </v0_generation>
  <visual_capture>
    <required>true</required>
    <screenshot_path>tests/visual/__screenshots__/{screen-name}/</screenshot_path>
    <capture_command>npx playwright test --project=visual-capture --update-snapshots</capture_command>
  </visual_capture>
  <instructions>
    1. Check component catalog for V0 prompts before building
    2. Use V0 MCP tools (v0_generate_ui, v0_chat_complete) for presentational components
    3. Include design system preamble in all V0 prompts
    4. Capture screenshots after implementation for visual verification
    5. Include screenshot paths in deliverables
  </instructions>
</ui_context>
```
...
- Set `tasks[task_id].implementer_type = "{selected implementer}"` (e.g., "frontend-implementer")
...
**For UI/Frontend Tasks - Visual Verification:**

When verifying frontend-implementer work, `verify-app` performs VISUAL verification:
...
1. Review screenshots against wireframes (layout, component placement)
2. Verify design tokens are used (OKLCH colors, Geist font, spacing)
3. Check component styling matches design spec
4. Look for visual glitches, overflow, alignment issues

If visual issues found:
- Provide specific feedback with file:line references
- Include suggested V0 refinement prompts where applicable
- Return visual_issues[] in your response
</instructions>
```

Visual verification is BLOCKING for UI tasks - implementation cannot proceed to SIMPLIFY until visual criteria pass.

**If visual issues found:** Return to `frontend-implementer` with visual feedback (NOT to DEBUG stage):
...
After visual fixes, return to VERIFY stage for re-verification (visual + functional).

**If functional tests pass (and visual passes for UI tasks):** Proceed to SIMPLIFY stage.

**If functional tests fail and strategy requires blocking:** Proceed to DEBUG stage.
...
**If REJECTED:** Loop back to IMPLEMENT stage with the reviewer's feedback.

**IMPORTANT:** Use the SAME implementer type that performed the original implementation
(e.g., if `frontend-implementer` built the feature, `frontend-implementer` fixes the issues).

Track the original implementer in state: `tasks[task_id].implementer_type`

**Delegation prompt for rejection fix:**
```xml
...
```json
{
  ...
  "tasks": {
    "TRD-XXX": {
      ...
      "cycle_position": "implement|verify|verify_red|debug|simplify|verify_post_simplify|review|update_artifacts|complete",
