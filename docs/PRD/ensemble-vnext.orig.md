...
- **Vendored runtime** - Consistent behavior across local CLI, web, and cloud sessions through a committed `.claude/` directory
- **Staged execution with verification gates** - Implement -> Verify -> Simplify -> Verify -> Review cycle with automated quality checks
...
**For organizations**: Unlock the full potential of Claude Code across entire engineering teams, not just power users, multiplying ROI on AI tooling investment.
...
- Translate implicit expert workflows (like those of Boris Jabes) into explicit, documented commands
...
- Require forking or patching Claude Code
...
- No cloud services beyond Claude's built-in remote execution
- No additional authentication systems
...
- Teams can work outside the framework when appropriate
...
- Not designed for non-engineering use cases
...
| F1.2 `/rebase-project` | Upgrade vendored runtime to newer plugin version | P0 |
| F1.3 `/add-skill` | Add additional skills without full rebase | P0 |
...
4. **Implementation Approach**:
   - Phase 1: Shell script performs scaffolding (creates/overwrites `.claude/` structure)
   - Phase 2: Interactive prompts handle user confirmations for updates
   - Phase 3: Validation shell script verifies file structure is correct (deterministic check)
...
- Create project-appropriate subagent roster
- Compile skills based on detected tech stack
...
- Compile and trim skill for project context
...
| F2.3 Skills Compilation | Pull and trim skills based on tech stack | P0 |
| F2.4 Commands Generation | Create workflow commands in .claude/commands/ | P0 |
...
| Feature | Description | Priority |
|---------|-------------|----------|
| F5.1 `/update-project` | Manual learning capture with Constitution updates | P0 |
| F5.2 `/cleanup-project` | Review and prune CLAUDE.md and project artifacts | P1 |
...
- Archive old PRDs/TRDs to `docs/archive/`
...
|------|-------|---------|----------|
| F6.1 Permitter | PermissionRequest | Smart bash permission expansion | P0 |
| F6.2 Router | UserPromptSubmit | Route prompts to appropriate commands/skills | P0 |
| F6.3 Formatter | PostToolUse | Format code output (project-configurable) | P0 |
| F6.4 Status | SubagentStop | Update status artifact on completion | P0 |
| F6.5 Learning | SessionEnd | Auto-capture learnings to CLAUDE.md | P0 |
...
| Feature | Description | Priority |
|---------|-------------|----------|
| F8.1 Review CLI | Submit artifacts to secondary LLM for critique | P1 |
| F8.2 PRD Review | Send (Story + PRD) for product review | P1 |
| F8.3 TRD Review | Send (PRD + TRD) for technical review | P1 |
...
| Feature | Description | Priority |
|---------|-------------|----------|
| F9.1 Stop Hook | Enforce completion criteria in autonomy mode | P1 |
| F9.2 Retry Logic | Automatic retry with debug loop (2-3 attempts) | P1 |
...
#### F10: External Review CLI

| Feature | Description | Priority |
|---------|-------------|----------|
| F10.1 Review Command | CLI tool for external artifact review | P2 |
| F10.2 Diff Integration | Show changes since last review | P2 |

#### F11: Monorepo Support

| Feature | Description | Priority |
|---------|-------------|----------|
| F11.1 Multi-Package Detection | Identify monorepo structure | P2 |
| F11.2 Package-Specific Skills | Different skills per package | P2 |
| F11.3 Cross-Package Coordination | Manage dependencies between packages | P2 |

#### F12: Remote Session Teleport Integration

| Feature | Description | Priority |
|---------|-------------|----------|
| F12.1 `/teleport` Command | Pull completed work from remote sessions | P2 |
| F12.2 Session Coordination | Coordinate multiple remote sessions | P2 |
...
| TR2.3 | Identical behavior in CLI, web, and cloud sessions |
| TR2.4 | Local settings (`.local.json`) gitignored |
| TR2.5 | Version-controlled customizations reviewable in PRs |
...
| TR3.3 | Hooks never block unless explicitly required (Completion Gate) |
...
| TR5.4 | Status files gitignored by default (operational, not source) |
